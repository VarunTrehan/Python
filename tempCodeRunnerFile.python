from flask import Flask,render_template,request,session,redirect,url_for,flashfrom flask_sqlalchemy import SQLAlchemy
from flask_login import UserMixin
from werkzeug.security import generate_password_hash,check_password_hashfrom flask_login import login_user,logout_user,login_manager,LoginManagerfrom flask_login import login_required,current_user
# MY db connection
local_server= True
app = Flask(_name_)
app.secret_key='vst'
...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/vstdbms'
db=SQLAlchemy(app)
...
@login_required
def delete(rid):
# db.engine.execute(f"DELETE FROM register WHERE register.rid={rid}")post=Register.query.filter_by(rid=rid).first()
...
farmername='{farmername}',adharnumber='{adharnumber}',age='{age}',gender='{gender}',phonenumber='{phonenumber}',address='{address}',farming='{farmingtype}'")
...
(username,email,password) VALUES ('{username}','{email}','{encpassword}')")# this is method 2 to save data in db
...
(farmername,adharnumber,age,gender,phonenumber,address,farming) VALUES('{farmername}','{adharnumber}','{age}','{gender}','{phonenumber}','{address}','{farmingtype}')")
